...
public class Components {
	 ...
	 *            是否有缓存 ：true、false
	 */
	public void testFirstLogin(AndroidDriver driver, String netWorkType,
			User user, boolean noCache) {
		if (noCache) {
			driver.resetApp();
		}
	...
	// * @param user 用户对象
	// */
	// public void testFirstLogin(AndroidDriver driver,String networkType,User
	// user)
	// {
	 ...
	 * @return
	 */
	public void testLoginDelay(AndroidDriver driver, String networkType,
			User user, int i) {
		...
		Log.info("点击开始体验");
		if (!elementManager.doesElementDisplay(driver,
				By.id("cn.cj.pe:id/hjl_headicon"))) {
			driver.sendKeyEvent(3);
			CommonUtil.sleep(2000);

			driver.findElement(By.name("139邮箱")).click();
			CommonUtil.sleep(2000);
		}
		Log.info("点击【主菜单】");
		// 点击【主菜单】
		...
		// 经常找不到设置
		if (!elementManager.doesElementDisplay(driver, By.name("设置"))) {
			driver.sendKeyEvent(3);
			CommonUtil.sleep(2000);
			...
			// 点击【主菜单】
			elementManager.waitForElement(driver,
					By.id("cn.cj.pe:id/hjl_headicon")).click();
			CommonUtil.sleep(1000);
			driver.swipe(width / 3, height * (4 / 5), width / 3, height / 5,
					500);
			CommonUtil.sleep(1000);
			driver.swipe(width / 3, height * (4 / 5), width / 3, height / 5,
					500);
